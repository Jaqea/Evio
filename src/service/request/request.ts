...
export function createRequest(
  ...
  function put<T>(url: string, data: any, config?: AxiosRequestConfig) {
    return asyncRequest<T>({ url, data, method: "put", axiosConfig: config });
  }
   ...
   * @param data - 请求body的data
   */
  function getFetch(url: string, method: RequestMethod, data?: any) {
    return fetch(`${axiosConfig.baseURL}${url}`, {
      method: method,
      body: data,
    });
  }
...
export function createHookRequest(
   ...
   * axiosConfig - axios配置
   */
  function useRequest<T>(param: RequestParam): RequestResultHook {}
}
